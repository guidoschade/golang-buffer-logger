...
import ( "log"; "net"; "fmt"; "time"; "strings" )
...
func mlog(level int, a ...interface{}) {
  if level <= debug {
    ...
    log_index ++;
    if log_index >= log_history { log_index = 0; }
...
func clear_logs() {
  ...
  for i := 0; i<log_history; i++ {
    logs[i].message = ""; logs[i].level = 0; logs[i].timestamp = 0;
  }
}
...
func show_logs() {
	...
	log.Printf("{ \"id\":%d, \"ts\":%d, \"level\": %d, \"message\": \"%s\" }", x, logs[x].timestamp, logs[x].level, strings.TrimSpace(logs[x].message));
      }
    }
